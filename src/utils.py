...
def save_simple_metrics_report(train_score: float,test_score: float,validation_score: float,model: Pipeline)-> None:
    with open('report.txt','w') as report_file:
        report_file.write('# Model Pipeline Description') # usamos el # por que sera visto en formato markdown
...
def get_model_performance_test_set(y_real:pd.Series,y_pred:pd.Series) -> None:
    ...
    ax.set_title('Accuracy or R2')
    fig.savefig('prediction_behaviour.png')
    print('La grafica ha sido creada exitosamente')
